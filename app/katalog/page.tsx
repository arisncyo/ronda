  ...
  return (
    <main className="min-h-screen bg-[#f8fafc] pt-[70px] pb-16">
      ...
      <section className="max-w-6xl mx-auto px-4 mt-10 mb-8">
        ...
        <div className="bg-white rounded-2xl p-4 border border-[#eef2f6] flex flex-wrap gap-2 items-center justify-center mt-4">
          <span className="text-xs font-semibold text-[#50595A] mr-1">Merk:</span>
          {brands.map((brand) => (
            <button
              key={brand}
              type="button"
        ...
        {filteredProducts.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {filteredProducts.map((product) => (
              <div
                key={product.id}
                className="group bg-white rounded-2xl overflow-hidden border border-[#eef2f6] hover:shadow-lg hover:-translate-y-1 transition-all duration-300 flex flex-col"
              >
                <Link
                ...
                >
                  <div className="relative h-40 w-full bg-slate-100 overflow-hidden">
                    ...
                    />
                    <span className="absolute top-3 left-3 bg-white/90 backdrop-blur-sm text-[#042327] text-[11px] font-bold px-2.5 py-1 rounded-lg">
                      {product.brand}
                    </span>
                    <span className="absolute top-3 right-3 bg-[#1888CD] text-white text-[11px] font-bold px-2.5 py-1 rounded-lg">
                      {product.category === "Kamera CCTV" ? "Kamera" : product.category}
                    </span>
                  </div>

                  <div className="p-4 flex flex-col flex-grow">
                    <h3 className="text-base font-bold text-[#042327] mb-1.5 leading-snug group-hover:text-[#1888CD] transition-colors">
                      {product.name}
                    </h3>

                    <p className="text-xs text-[#50595A] leading-relaxed mb-3 line-clamp-2">
                      {product.description}
                    </p>
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-[#eef2f6]">
                      <div>
                        {product.hasPrice ? (
                          <span className="text-base font-extrabold text-[#1888CD]">
                            {product.price}
                          </span>
                        ) : (
                          <span className="text-xs font-bold text-[#50595A]">
                            Hubungi untuk harga
                          </span>
                        )}
                      </div>
                      <span className="text-xs font-semibold text-[#1888CD] group-hover:gap-2 inline-flex items-center gap-1 transition-all">
                        Detail
      ...
      <section className="max-w-6xl mx-auto px-4 mt-16">
        <div className="relative rounded-3xl bg-gradient-to-r from-[#1888CD] to-[#0a5a8a] text-white p-8 md:p-12 overflow-hidden shadow-lg">
          <div className="relative z-10 text-center max-w-xl mx-auto">
            ...
            <a
              ...
              className="inline-flex items-center gap-2 bg-white hover:bg-[#f0f4f8] text-[#1888CD] font-bold text-base px-8 py-3.5 rounded-2xl transition-all duration-300 no-underline shadow-lg"
            >
